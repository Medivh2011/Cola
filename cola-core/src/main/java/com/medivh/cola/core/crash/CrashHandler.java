...
//			uploadExceptionToServer(file);
	...
	private File dumpExceptionToSDCard(Throwable ex) throws IOException
	{
		...
		File dir = getDiskCacheDir(mContext, "Crash");
		if (!dir.exists())
		{
			dir.mkdirs();
		}
		...
		File file = new File(getDiskCacheDir(mContext, "Crash").getPath() +File.separator+ FILE_NAME + time + FILE_NAME_SUFFIX);
		try
		{
	...
	// 设置上传到服务器的接口回调
	public void setCrashReporter(HttpReportCallback callback)
	{
